...
export interface VocabularyWord {
  ...
  isAiGenerated?: boolean;     // Flag to indicate if definition was generated by AI
}
...
export type CodeLanguage = 'java' | 'python' | 'c' | 'cpp' | 'javascript' | 'typescript' | 'go' | 'kotlin';
export type SubmissionStatus = 'Accepted' | 'Wrong Answer' | 'Runtime Error' | 'Compilation Error' | 'Time Limit Exceeded';
...
export interface CodeQuestion {
  ...
  // Custom & Spaced Repetition tracking fields
  userId?: string;
  topicId?: string;
  isCustom?: boolean;
  easeFactor?: number;
...
export interface CodingProgress {
  id: string; // Format: `${userId}_${questionId}`
  userId: string;
  questionId: string;
  topicId?: string;
  easeFactor: number;
